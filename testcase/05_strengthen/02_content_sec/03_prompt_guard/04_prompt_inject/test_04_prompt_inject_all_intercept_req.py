...
class TestPromptInjectAllInterceptReq(seldom.TestCase):
    """增强管理-内容安全-提示词注入防护:所有规则-开启-拦截-提问命中"""
    ...
    def start_class(cls):
        ...
        # 加载测试数据
        with open("testcase/05_strengthen/02_content_sec/03_prompt_guard/04_prompt_inject/prompt_inject_all_data.json", "r",
                  encoding="utf-8") as f:
            json_data = json.load(f)
        cls.test_data = json_data.get("5-2-3-prompt-inject-all-004", [])

    def test_prompt_inject_all_intercept_req(self):
        """调用大模型，发起请求"""
        # 确保有测试数据
