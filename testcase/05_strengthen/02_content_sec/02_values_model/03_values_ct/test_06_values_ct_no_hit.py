...
class TestValuesCTNoHit(seldom.TestCase):
    """增强管理-内容安全-价值观模型监测:监测上下文-开启-未命中"""
    ...
    # 第一个方法：调用大模型，发起请求
    @seldom.file_data("values_ct_data.json", key="5-2-2-values-ct-006")
    def test_values_ct_no_hit(self, scene, req, resp):
        """调用大模型，发起请求"""
        # 1. 发送API请求
