...
class TestValuesAllDetectRespHitNoStream(seldom.TestCase):
    """增强管理-内容安全-价值观模型监测:所有规则-开启-监测-回答命中-非流式"""
    ...
    # 第一个方法：收集所有测试数据
    @seldom.file_data("values_all_data.json", key="5-2-2-values-all-004")
    def test_values_all_detect_resp_hit_no_stream(self, scene, req, resp):
        """调用大模型，发起请求"""
        # 1. 发送API请求
