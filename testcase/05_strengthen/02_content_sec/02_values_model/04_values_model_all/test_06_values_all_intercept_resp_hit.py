...
class TestValuesAllInterceptRespHit(seldom.TestCase):
    """增强管理-内容安全-价值观模型监测:所有规则-开启-拦截-回答命中-流式"""
    ...
    # 第一个方法：收集所有测试数据
    @seldom.file_data("values_all_data.json", key="5-2-2-values-all-006")
    def test_values_all_intercept_resp_hit(self, scene, req, resp):
        """调用大模型，发起请求"""
        # 1. 发送API请求
