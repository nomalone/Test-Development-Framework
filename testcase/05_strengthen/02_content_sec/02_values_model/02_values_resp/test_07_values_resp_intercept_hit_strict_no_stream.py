...
class TestValuesRespInterceptHitStrict(seldom.TestCase):
    """增强管理-内容安全-价值观模型监测:回答-开启-拦截-命中-严格-非流式"""
    ...
    @seldom.file_data("values_resp_data.json", key="5-2-2-values-resp-007")
    def test_values_resp_intercept_hit_strict_no_stream(self, scene, req, resp):
        """调用大模型，发起请求"""
        # 1. 发送API请求
