...
class TestKeywordReqIntercept(seldom.TestCase):
    """增强管理-内容安全-关键词监测:提问-开启-拦截"""
    ...
    def start_class(cls):
        ...
        # 加载测试数据
        with open("testcase/05_strengthen/02_content_sec/01_keyword_guard/01_keyword_req/keyword_req_data.json", "r",
                  encoding="utf-8") as f:
            json_data = json.load(f)
        cls.test_data = json_data.get("5-2-1-keyword-req-intercept", [])

    def test_keyword_req_intercept(self):
        """调用大模型，发起请求"""
        # 确保有测试数据
